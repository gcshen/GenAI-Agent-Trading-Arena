...
class Person:
    ...
    def query_memory(self, virtual_date)->list:
    ...
    def query_person(self, virtual_date)->list:
        cmd = "select * from person where virtual_date ={} and person_id ={}".format(
            virtual_date, self.person_id
        )
        self.db.execute_sql(cmd)
        results = self.db.fetchall()
        res_person_list = parse_persons(results) ## BUG: parse_persons -> List[Dict]
        return_person = []
        for i,p in enumerate(res_person_list):
            date = p["virtual_date"]
            cash = p["cash"]
