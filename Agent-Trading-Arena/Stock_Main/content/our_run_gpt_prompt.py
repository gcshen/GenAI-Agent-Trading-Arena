...
def integrate_gossip_info(virtual_date, persona):
    ...
    for m in memory:
        f = open(
            _paths.PROMPT_PATH.joinpath("gossip_info.txt"),
            "r"
        )
...
def integrate_stock_info(virtual_date, stocks_list):
    ...
    for stock in stocks_list:
        f = open(
            _paths.PROMPT_PATH.joinpath("stock_information.txt"),
            "r",
            encoding="utf-8",
        )
...
def integrate_hold_info(virtual_date, persona):
    ...
    if len(hold_list) == 0:
        ...
        for hold in hold_list:
            f = open(_paths.PROMPT_PATH.joinpath("hold_information.txt"), "r")
...
def integrate_reflect_info(virtual_date, persona:Person):
    iteration=0
    pre_reflect_info = ""
    while virtual_date>=0 and iteration<3:
        ...
        for m in memory:
            f = open(
                _paths.PROMPT_PATH.joinpath("reflect_info.txt"),
                "r"
            )
...
def update_strategy(virtual_date, persona, w_s, suggestion):
    ...
    prompt_template = _paths.PROMPT_PATH.joinpath("reflect.txt")
    prompt_input = create_prompt_input(virtual_date, persona, w_s, suggestion)
    prompt = generate_prompt(prompt_input, prompt_template)

    # with open("relect.txt", "w") as file: ##BUG
    with open(prompt_template, "w") as file:
        file.write(prompt)
...
def long_reflect(virtual_date, persona):
    ...
    with open(_paths.PROMPT_PATH.joinpath("pre_reflect.txt"), "w") as file:
        file.write(prompt)
...
def pre_reflect(virtual_date, persona):
    ...
    prompt_template = _paths.PROMPT_PATH.joinpath("pre_reflect.txt") #只有一个input
    prompt_input = create_prompt_input(virtual_date, persona)
    prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_generate_gossip(virtual_date, persona):
    ...
    prompt_template = _paths.PROMPT_PATH.joinpath("gossip.txt")
    prompt_input = create_prompt_input(virtual_date, persona)
    prompt = generate_prompt(prompt_input, prompt_template)

    with open(_paths.PROMPT_PATH.joinpath("gossip.txt"), "w") as file:
        file.write(prompt)
...
def analysis(virtual_date, persona, stocks_list, market_index, analysis_num, gossip_num_max):
    ...
    prompt_template = _paths.PROMPT_PATH.joinpath("analysis.txt")
...
def run_gpt_prompt_choose_sell_stock(
    ...
    prompt_template = _paths.PROMPT_PATH.joinpath("sell_based_on_analysis.txt")
